...
| Model | Pretraining | Training Strategy | Accuracy |
| :--- | :--- | :--- | :---: |
| **Baseline (DeepFace)** | FER-2013 | Zero-shot / Frozen | 40.00% |
| **Ours (ConvNeXt)** | ImageNet-1K | **Full Unfreeze + Augmentation** | **98.37%** 🚀 |
...
選用了奧運金牌得主郭婞淳的 Vlog 影片 `(vlog.mp4)` 進行測試，以評估模型在 **動態真實場景** 中的泛化能力。
...
### 3. 結論 (Conclusion)
    此次結果凸顯了現有 7 分類模型在處理複雜人類情感時的局限性。未來的改進方向可引入 **多標籤分類 (Multi-Label Classification)** 或定義如「驚喜 (Happily Surprised) = 驚喜 + 開心」等複合類別。
